...
var Random = function Random(seed) {
  ...
  var random = function random() {
    return (random.raw() - 1) / 2147483646;
  }; // 0-1
  ...
  Object.assign(random, {
    seed: seed,
    int: function int() {
      var min = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 2147483647;
    ...
    getNextSeed: function getNextSeed() {
      return n_seed = n_seed * 16807 % 2147483647;
    },
    // 0-2147483646
    ...
    reset: function reset() {
      ...
      if (_current <= 0) _current += 2147483646; // for SEED < 10,000 the first number is always ~ 0.01, so let's burn that one
    ...
    shuffle: function shuffle(array) {
      var i = array.length,
          temp,
          i_rand; // While there remain elements to shuffle...

      while (0 !== i) {
        // Pick a remaining element...
        i_rand = Math.floor(random() * i);
        i -= 1; // And swap it with the current element.
  ...
  return (
    ...
    function (_superclass) {
      ...
      function Random() {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Random).call(this, opts));

        if (opts._prng) {
          // derive seed from a parent PRNG
          _this._SEED = opts._prng.random.getNextSeed();
        } else {
          _this._SEED = opts._SEED || opts.seed;
        }

        _this.random = Random(_this._SEED);
        return _this;
      }
...
},{}],"histogram.js":[function(require,module,exports) {
"use strict";
...
var binRandom = function binRandom(numbers) {
  var counts = _lodash.default.range(10).map(function (_i) {
    return 0;
  });

  numbers.forEach(function (n) {
    return counts[Math.floor(n * 10)]++;
  });
  return counts;
};

var _default = function _default(datasets, querySelector) {
  var series = datasets.map(function (data) {
    return binRandom(data);
  });

  var labels = _lodash.default.range(10).map(function (_i) {
    return (_i * 0.1).toFixed(1);
  });
...
exports.default = _default;
},{"lodash":"../node_modules/lodash/lodash.js"}],"plots.js":[function(require,module,exports) {
"use strict";
...
var Random = _index.default;
var range = _lodash.default.range;

function makeNthSeries() {
  var n_series = 4;
  var nth_digits = range(n_series).map(function (_i) {
    return [];
  });
  range(10000).map(function () {
    var r = Random(Math.floor(Math.random() * 1000));
    range(n_series).map(function (nth) {
      return nth_digits[nth].push(r());
    });
  });
  return nth_digits;
}
...
function showUsage() {
  ...
  var rows = [[function () {
    return random();
  }, "A float [0,0.999...]"], [function () {
    return random.int();
  }, "An int [0,2147483646]"], [function () {
    return random.int(10);
  }, "I'm thinking of a number between 0 and 9"], [function () {
    return random.int(1, 11);
  }, "I'm thinking of a number between 1 and 10"], [function () {
    return random.choice(["heads", "tails"]);
  }, "Flip a coin!"], [function () {
    return random.shuffle(range(5));
  }, "Shuffled array, [0-4]"], [function () {
    return random.reset();
  }, "Reset the PRNG"], [function () {
    return random();
...
exports.default = _default;
},{"lodash":"../node_modules/lodash/lodash.js","../index":"../index.js"}],"ur-random.tag":[function(require,module,exports) {
"use strict";
...
var _plots = _interopRequireDefault(require("./plots"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var riot = require('riot');

riot.tag2('ur-random', '<section> <h2>Usage: <code class="language-js">Random(SEED)</code></h2> <p> A new PRNG with seed 1234 would be instantiated with <code class="language-js">const random = Random(1234)</code>. The result is a function that returns a float between 0 and 0.999... (in order to match the api of <code class="language-js">Math.random()</code>). </p> <p> The function also has several built in functions: </p> <table class="table table-striped"> <thead> <th>Code</th> <th class="hide-sm">Result</th> <th>Description</th> <tr each="{row,i in usage_rows}"> <td> <code class="language-js">{row[0]}</code> </td> <td class="hide-sm">{row[1]}</td> <td>{row[2]}</td> </tr> </table> </section> <section> <h2>Usage: <code class="language-js">Random(SEED).getNextSeed()</code></h2> <p> The impetus behind this library was to use a seeded PRNG to make a video game replayable but chaotic. With one PRNG for the whole game, the butterfly effect takes over, and killing a character or ending a level one turn early can drastically alter everything controlled by the PRNG down the line. To avoid this, the PRNG has a second stream of randomness accessible via <code class="language-js">random.getNextSeed()</code>. In short, both of the following have the same results: </p> <div class="columns"> <div class="{col}"> <pre><code class="language-js">{mixedSeeds_string}</code></pre> </div> <div class="{col}"> <pre><code class="language-js">{seedFirst_string}</code></pre> </div> </div> </section> <section> <h2>Usage: <code class="language-js">Random.Mixin</code></h2> <p> This is an ES6 mixin to easily create classes that have access to a PRNG ala <code class="language-js">this.random</code> and can create children with their own PRNG seeded by the parent class. So, for example, a game could have a seed, which generates levels, which generate enemies, which move randomly and drop random items. Note that <code class="language-js">Random.Mixin</code> can be used with or without a parent class as it\'s first argument defaults to JavaScript\'s built in Object. </p> <div class="columns"> <div class="{col}"> <pre><code class="language-js">{classExample_string}</code></pre> </div> </div> </section> <section class="distribution"> <h2>Test: Distribution of numbers</h2> <p> This is to verify that the the first 10,000 numbers of <code class="language-js">Random(456)</code> form a nearly uniform distribution. I might make this more rigorous in the future when I add formal tests. </p> <div class="columns"> <div class="{col}"> <pre><code class="language-js">{makeDistribution_string}</code></pre> </div> <div class="{col}"> <div id="dist_chart"></div> </div> </div> </section> <section> <h2>Test: Distribution of nth number</h2> <p> There was <a href="https://gist.github.com/blixt/f17b47c62508be59987b#gistcomment-1272204">a bug</a> in the PRNG this library is based on. The first number was always very small (about 0.04) for the first value if the seed was small (less than 10,000). To combat this the first value is discarded when the <code class="language-js">Random(SEED)</code> object is initiated. </p> <p> The following shows the 1st, 2nd, 3rd, and 4th value given by a PRNG seeded with a number less than 1,000 </p> <pre><code class="language-js">{makeNthSeries_string}</code></pre> <div class="nth-chart chart"> <div id="nth_chart"></div> <div class="legend"> <div each="{l,i in legends}"> <span class="series-legend {l}"></span> {legend_labels[i]} number </div> </div> </div> </section>', '', '', function (opts) {
  ...
  for (var key in _plots.default) {
    this[key + "_string"] = _plots.default[key].toString();
  }

  this.usage_rows = _plots.default.showUsage();
  this.on("mount", function () {
    (0, _histogram.default)([_plots.default.makeDistribution()], "#dist_chart");
    (0, _histogram.default)(_plots.default.makeNthSeries(), "#nth_chart");
  });
...
if (document.querySelector("ur-random")) {
  ...
  var tag = document.createElement("ur-random");
  document.querySelector("#content").appendChild(tag);
  riot.mount(tag);
}
},{"riot":"../node_modules/riot/riot.js","lodash":"../node_modules/lodash/lodash.js","../index":"../index.js","./histogram":"histogram.js","./plots":"plots.js"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","ur-random.tag"], null)
//# sourceMappingURL=/ur-random/ur-random.bcc5f280.map